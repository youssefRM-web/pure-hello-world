import React, { useState } from "react";
import { useGroupsQuery } from "@/hooks/queries";
import { Skeleton } from "@/components/ui/skeleton";
import {
  Table,
  TableBody,
  TableCell,
  TableHead,
  TableHeader,
  TableRow,
} from "@/components/ui/table";
import SpaceGroupDetailsModal from "./SpaceGroupDetailsModal";
import { useQueryClient } from "@tanstack/react-query";
import { Package } from "lucide-react";
import { useLanguage } from "@/contexts/LanguageContext";

const SpaceGroupsTable = () => {
  const { data: groups, isLoading } = useGroupsQuery();
  const [selectedGroupId, setSelectedGroupId] = useState<string | null>(null);
  const [isDetailsModalOpen, setIsDetailsModalOpen] = useState(false);
  const { t } = useLanguage();

  const spaceGroups =
    groups?.filter(
      (group) =>
        group?.spaces &&
        group?.spaces?.length >= 0 &&
        group?.belongTo === "spaces",
    ) || [];

  const handleGroupClick = (groupId: string) => {
    setSelectedGroupId(groupId);
    setIsDetailsModalOpen(true);
  };

  const SkeletonRow = () => (
    <TableRow>
      <TableCell>
        <Skeleton className="h-8 w-8" />
      </TableCell>
      <TableCell>
        <Skeleton className="h-4 w-40" />
      </TableCell>
      <TableCell className="text-center">
        <Skeleton className="h-4 w-8 mx-auto" />
      </TableCell>
      <TableCell className="text-center">
        <Skeleton className="h-6 w-6 mx-auto rounded-full" />
      </TableCell>
      <TableCell className="text-center">
        <Skeleton className="h-4 w-8 mx-auto" />
      </TableCell>
      <TableCell className="text-center">
        <Skeleton className="h-4 w-8 mx-auto" />
      </TableCell>
    </TableRow>
  );

  return (
    <div className="bg-card rounded-lg border-border w-full">
      <div className="overflow-x-auto">
        <div className="min-w-full inline-block align-middle">
          <Table className="min-w-[600px]">
            <TableHeader>
              <TableRow>
                <TableHead className="w-8"></TableHead>
                <TableHead>{t("spaces.name")}</TableHead>
                <TableHead className="text-center">
                  {t("spaces.linkedSpaces")}
                </TableHead>
                <TableHead className="text-center">
                  {t("spaces.open")}
                </TableHead>
                <TableHead className="text-center">
                  {t("spaces.inProgress")}
                </TableHead>
                <TableHead className="text-center">
                  {t("spaces.completed")}
                </TableHead>
              </TableRow>
            </TableHeader>

            <TableBody className="bg-card divide-y divide-border">
              {isLoading ? (
                Array.from({ length: 5 }).map((_, index) => (
                  <SkeletonRow key={index} />
                ))
              ) : spaceGroups &&
                spaceGroups.find((g) => g.belongTo === "spaces") &&
                spaceGroups.length >= 0 ? (
                spaceGroups?.map((group) => (
                  <TableRow
                    key={group._id}
                    className="hover:bg-accent/50 cursor-pointer"
                    onClick={() => handleGroupClick(group?._id)}
                  >
                    <TableCell className="px-3 sm:px-6 py-4 whitespace-nowrap">
                      <div className="bg-[#F1F5FE] w-8 h-8 rounded-md flex items-center justify-center">
                        <svg
                          className="w-6 h-6"
                          fill="#4D81ED"
                          xmlns="http://www.w3.org/2000/svg"
                          width={24}
                          height={24}
                          viewBox="0 0 24 24"
                        >
                          <path d="M10 3H4C3.447 3 3 3.447 3 4v6c0 .553.447 1 1 1h6c.553 0 1-.447 1-1V4C11 3.447 10.553 3 10 3zM9 9H5V5h4V9zM20 3h-6c-.553 0-1 .447-1 1v6c0 .553.447 1 1 1h6c.553 0 1-.447 1-1V4C21 3.447 20.553 3 20 3zM19 9h-4V5h4V9zM10 13H4c-.553 0-1 .447-1 1v6c0 .553.447 1 1 1h6c.553 0 1-.447 1-1v-6C11 13.447 10.553 13 10 13zM9 19H5v-4h4V19zM17 13c-2.206 0-4 1.794-4 4s1.794 4 4 4 4-1.794 4-4S19.206 13 17 13zM17 19c-1.103 0-2-.897-2-2s.897-2 2-2 2 .897 2 2S18.103 19 17 19z" />
                        </svg>
                      </div>
                    </TableCell>
                    <TableCell className="px-3 sm:px-6 py-4 whitespace-nowrap font-medium text-foreground first-letter:uppercase">
                      {group?.name}
                    </TableCell>
                    <TableCell className="px-3 sm:px-6 py-4 whitespace-nowrap text-center text-sm text-foreground">
                      {
                        (group?.spaces || []).filter((s: any) => !s?.archived)
                          .length
                      }
                    </TableCell>
                    <TableCell className="px-3 sm:px-6 py-4 whitespace-nowrap text-center">
                      {group?.taskCounts?.open > 0 ? (
                        <span className="inline-flex items-center justify-center w-6 h-6 bg-orange-100 text-orange-600 rounded-full text-xs font-medium">
                          {group?.taskCounts?.open}
                        </span>
                      ) : (
                        <span className="text-muted-foreground">
                          {group?.taskCounts?.open}
                        </span>
                      )}
                    </TableCell>
                    <TableCell className="px-3 sm:px-6 py-4 whitespace-nowrap text-center text-sm text-foreground">
                      {group?.taskCounts?.inProgress}
                    </TableCell>
                    <TableCell className="px-3 sm:px-6 py-4 whitespace-nowrap text-center text-sm text-foreground">
                      {group?.taskCounts?.completed}
                    </TableCell>
                  </TableRow>
                ))
              ) : (
                <TableRow>
                  <TableCell
                    colSpan={12}
                    className="text-center py-12 text-muted-foreground"
                  >
                    <Package className="w-12 h-12 mx-auto mb-3 text-gray-300" />
                    <p>{t("spaces.noSpaceGroupsFound")}</p>
                  </TableCell>
                </TableRow>
              )}
            </TableBody>
          </Table>
        </div>
      </div>

      <SpaceGroupDetailsModal
        isOpen={isDetailsModalOpen}
        onClose={() => setIsDetailsModalOpen(false)}
        groupId={selectedGroupId}
      />
    </div>
  );
};

export default SpaceGroupsTable;
