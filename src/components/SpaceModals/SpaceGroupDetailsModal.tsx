...
import { X, Trash2, ChevronRight, DoorClosed, Menu, Edit } from "lucide-react";
...
import DeleteSpaceGroupModal from "./DeleteSpaceGroupModal";
import LinkedSpaceTasksModal from "./LinkedSpaceTasksModal";
import LinkedSpacesModal from "./LinkedSpacesModal";
import { useQueryClient } from "@tanstack/react-query";
import { useLanguage } from "@/contexts/LanguageContext";

interface SpaceGroupDetailsModalProps {
  isOpen: boolean;
  onClose: () => void;
  groupId: string | null;
}

const SpaceGroupDetailsModal = ({
  isOpen,
  onClose,
  groupId,
}: SpaceGroupDetailsModalProps) => {
  ...
  const [isLinkedSpacesModalOpen, setIsLinkedSpacesModalOpen] = useState(false);
  ...
  const groupSpaces = group.spaces || [];
  const groupTasks = groupSpaces.flatMap(
    (space: any) => space.linkedTasks || [],
  );
  ...
  const handleSaveName = async () => {
    ...
    try {
      const response = await apiService.put(`/groups/${group._id}`, {
        name: editedName.trim(),
        spaces: group.spaces?.map((s) => s._id || s) || [],
      });
      setIsEditingName(false);
      queryClient.invalidateQueries({ queryKey: ["groups"] });
      refreshData();
      toast.success(t("spaces.title"), {
        description: t("spaces.groupNameUpdated"),
      });
      return response;
    } catch (error: any) {
      ...
      toast.error(t("common.error"), {
        description:
          error.response?.data?.message || t("spaces.failedUpdateGroup"),
      });
      return null;
    }
  };
  ...
  const groupSpacesTasks = groupSpaces.flatMap((gs) => gs.linkedTasks)
  ...
  return (
    <>
          ...
          <div className="space-y-2">
            <div className="flex items-center gap-3 pb-3 px-6 group">
              ...
              <div className="flex-1">
                <div className="flex items-center justify-between gap-2">
                  <div className="font-medium text-[#565E6CFF]">
                    {editedName}
                  </div>
                  <button
                ...
                <div className="fixed inset-0 z-50 flex items-center justify-center px-4">
                  ...
                  <div className="relative bg-background rounded-lg shadow-2xl border p-6 w-full max-w-md animate-in fade-in zoom-in-95 duration-200">
                    ...
                    <Input
                      ...
                      placeholder={t("spaces.enterGroupName")}
                      onKeyDown={(e) => {
            ...
            <div className="bg-accent/50">
              <div className="space-y-1 pt-4 pb-4 mx-6">
                ...
                <button
                  onClick={() => setIsLinkedSpacesModalOpen(true)}
                  className="w-full flex items-center justify-between p-3 bg-background border rounded-md hover:bg-accent/50 transition-colors"
                >
                  <div className="flex items-center gap-3">
                    <div className="bg-[#FDF5F1FF] w-8 h-8 rounded-md flex justify-center items-center">
                      <DoorClosed className="w-5 h-5 text-[#E1602CFF]" />
                    </div>
                    <span className="text-sm">{t("spaces.spaces")}</span>
                    <div className="flex items-center justify-center">
                      <CountBadge count={groupSpaces.length} />
                    </div>
                  </div>
                  <ChevronRight className="w-4 h-4 text-gray-400" />
                </button>
      ...
      />
      <LinkedSpacesModal
        isOpen={isLinkedSpacesModalOpen}
        onClose={() => setIsLinkedSpacesModalOpen(false)}
        groupId={group._id}
        groupName={group.name}
        spaces={groupSpaces}
      />
...
export default SpaceGroupDetailsModal;
