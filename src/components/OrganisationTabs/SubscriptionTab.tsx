...
const SubscriptionTab = () => {
  ...
  const { data: individualPlans = [], isLoading: individualPlansLoading } =
    useMyIndividualPlansQuery(organizationId);

  const activeBuildingCount = orgBuildings.filter(
    (b: any) => !b.archived,
  ).length;
  ...
  // A plan is considered "purchased" when the organization's currentPlan id
  // matches one of the individual plans assigned to this organization.
  const orgCurrentPlanId =
    typeof (organization as any)?.currentPlan === "string"
      ? (organization as any)?.currentPlan
      : (organization as any)?.currentPlan?._id;
  const matchedPurchasedPlan = individualPlans.find(
    (p: any) => p._id === orgCurrentPlanId,
  ) as any;
  // Enrich with subscription metadata coming from the organization document
  const purchasedIndividualPlan = matchedPurchasedPlan
    ? {
        ...matchedPurchasedPlan,
        nextBillingDate:
          matchedPurchasedPlan.nextBillingDate ||
          (organization as any)?.nextBillingDate ||
          (organization as any)?.subscriptionEndDate,
      }
    : (individualPlans.find((p: any) => p.purchasedAt) as any);
  const hasIndividualPlans =
    activeIndividualPlans.length > 0 && !organization?.currentPlan;
  const isPendingCancelled =
    (organization as any)?.subscriptionStatus === "pending_cancelled";
        ...
        description: t("organisation.downgradeSuccessDesc")
          ...
          .replace(
            ...
            data.nextBillingDate
              ? format(new Date(data.nextBillingDate), "dd.MM.yyyy")
              : "",
          ),
        variant: "success",
      });
  ...
  React.useEffect(() => {
    const handleExpiredSubscription = (event: CustomEvent) => {
      ...
      toast({
        ...
        description:
          event.detail?.message || t("organisation.subscriptionExpiredDesc"),
        variant: "destructive",
        duration: 10000,
      });
    };
    ...
    window.addEventListener(
      ...
      handleExpiredSubscription as EventListener,
    );
  ...
  if (hasIndividualPlans) {
    // Other active plans the customer could subscribe to (excluding the purchased one)
    const otherActivePlans = activeIndividualPlans.filter(
      (p: any) =>
        !purchasedIndividualPlan || p._id !== purchasedIndividualPlan._id,
    );

    // If a plan has been purchased, show that plan AND any other available plans
    if (purchasedIndividualPlan) {
      return (
        <div className="py-12 px-4 sm:px-6 lg:px-8 max-w-5xl mx-auto">
                ...
                <div className="flex items-center justify-center gap-2 flex-wrap">
                  <CardTitle className="text-2xl">
                    {purchasedIndividualPlan.nameKey
                      ? t(purchasedIndividualPlan.nameKey) ||
                  ...
                  {isPendingCancelled ? (
                    <Badge className="bg-destructive/10 text-destructive hover:bg-destructive/10 border-destructive/30 gap-1">
                      <Ban className="w-3.5 h-3.5" />
                      {t("organisation.cancelled") || "Cancelled"}
                    </Badge>
                  ) : (
                    <Badge className="bg-green-100 text-green-700 hover:bg-green-100 border-green-200 gap-1">
                      <BadgeCheck className="w-3.5 h-3.5" />
                      {t("organisation.purchased") || "Purchased"}
                    </Badge>
                  )}
                </div>
              </CardHeader>
              <CardContent className="space-y-6">
                <div className="text-center">
                  ...
                  {purchasedIndividualPlan.billingCycle === "yearly" && (
                    <p className="text-sm text-muted-foreground mt-1">
                      {t(
                        purchasedIndividualPlan.billingTextKey ||
                          "BILLED_YEARLY",
                      ) ||
                        t("plan.billedYearly") ||
                        "Billed yearly"}
                      : €
                      {(
                        purchasedIndividualPlan.monthlyDisplayPrice * 12
                      ).toLocaleString()}
                      /{t("plan.year") || "year"}
                    </p>
                  )}
                ...
                <div className="border border-border rounded-lg p-4 space-y-3">
                  ...
                  {purchasedIndividualPlan.nextBillingDate && (
                    <div className="flex justify-between text-sm">
                      ...
                      <span className="font-medium text-orange-600">
                        {format(
                          new Date(purchasedIndividualPlan.nextBillingDate),
                          "dd.MM.yyyy",
                        )}
                      </span>
                    </div>
                  ...
                  {((organization as any)?.subscriptionEndDate ||
                    ...
                    <div className="flex justify-between text-sm">
                      <span className="text-muted-foreground">
                        {t("organisation.subscriptionEndDate") ||
                ...
                {isPendingCancelled ? (
                  <div className="rounded-lg border border-destructive/30 bg-destructive/5 p-3 text-sm text-destructive text-center">
                    {t("organisation.willBeCancelledOn") ||
          ...
          {otherActivePlans.length > 0 && !isPendingCancelled && (
            <div className="mt-12">
              <div className="text-center mb-8">
                <h3 className="text-xl font-bold text-foreground mb-2">
                  {t("organisation.otherAvailablePlans") ||
                    "Other Available Plans"}
                </h3>
                <p className="text-sm text-muted-foreground">
                  {t("organisation.otherAvailablePlansDesc") ||
                    "Switch to another plan that fits your needs."}
                </p>
              </div>
              <div
                className={`grid gap-8 max-w-4xl mx-auto ${
                  otherActivePlans.length === 1
                    ? "grid-cols-1 max-w-md"
                    : "grid-cols-1 md:grid-cols-2"
                }`}
              >
                {otherActivePlans.map((plan: any) => {
                  const currentPlan = purchasedIndividualPlan as any;
                  let action: "process" | "upgrade" | "downgrade" = "process";
                  let buttonLabel = t("organisation.choosePlan") || "Subscribe";
                  if (currentPlan && currentPlan._id !== plan._id) {
                    if (plan.price > currentPlan.price) {
                      action = "upgrade";
                      buttonLabel = t("organisation.upgradePlan") || "Upgrade";
                    } else if (plan.price < currentPlan.price) {
                      action = "downgrade";
                      buttonLabel =
                        t("organisation.downgradePlan") || "Downgrade";
                    }
                  }
                  const checkoutPlan = {
                    _id: plan._id,
                    ...
                    billingTextKey: plan.billingTextKey,
                  };
                          ...
                          {plan.nameKey
                            ...
                            : plan.name ||
                              (plan.billingCycle === "monthly"
                                ? t("organisation.monthlyPlan") ||
                                  "Monthly Plan"
                                : t("organisation.yearlyPlan") ||
                                  "Yearly Plan")}
                        </CardTitle>
    ...
    return (
      <div className="py-12 px-4 sm:px-6 lg:px-8 max-w-5xl mx-auto">
        <div className="text-center mb-8">
          ...
          <p className="text-muted-foreground">
            {t("plan.INDIVIDUAL_PLAN_DESCRIPTION") ||
              "Choose the plan that works best for you. Prices are shown as monthly amounts for easy comparison."}
          </p>
        </div>

        <div
          className={`grid gap-8 max-w-4xl mx-auto ${activeIndividualPlans.length === 1 ? "grid-cols-1 max-w-md" : "grid-cols-1 md:grid-cols-2"}`}
        >
          {activeIndividualPlans.map((plan) => {
            // Determine action based on currently purchased plan
            const currentPlan = purchasedIndividualPlan as any;
            let action: "process" | "upgrade" | "downgrade" = "process";
            let buttonLabel = t("organisation.choosePlan") || "Subscribe";
            if (currentPlan && currentPlan._id !== plan._id) {
              if (plan.price > currentPlan.price) {
                ...
                action = "downgrade";
                buttonLabel = t("organisation.downgradePlan") || "Downgrade";
              }
            }
            const checkoutPlan = {
              _id: plan._id,
  ...
  const hasNoActiveSubscription = organization?.currentPlan == null;
  const isTrialExpired = subscriptionStatus?.status === "expired";
  const showContactSalesCard = isTrialExpired || !isTrial;
  ...
  if (!plans || plans.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center h-[60vh] text-center">
        ...
        <p className="text-muted-foreground">
          {t("organisation.noSubscriptionPlans")}
        </p>
      </div>
    );
  }
  ...
  return (
    <>
      <div className="py-12 px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto">
        ...
        <div
          className={`grid gap-6 md:gap-8 max-w-6xl mx-auto ${isTrial ? "grid-cols-1 max-w-md" : "grid-cols-1 md:grid-cols-2 xl:grid-cols-3"}`}
        >
          {plans
            ...
            ?.map((plan) => {
              ...
              const currentRank = currentPlanObj
                ? (planRank[currentPlanObj.name] ?? 0)
                : 0;
              const thisRank = planRank[plan.name] ?? 0;
              const isUpgrade =
                currentRank > 0 && thisRank > currentRank && !isCurrent;
              const isDowngrade =
                currentRank > 0 && thisRank < currentRank && !isCurrent;
              ...
              return (
                ...
                >
                  ...
                  <CardHeader
                    className={isTrial && isEnterprise ? "pb-4" : "pb-8"}
                  >
                    <CardTitle
                      className={`text-2xl font-bold ${isEnterprise ? "text-white" : "text-foreground"}`}
                    >
                    ...
                    {!(isTrial && isEnterprise) && (
                      <p
                        className={`text-base ${isEnterprise ? "text-white/70" : "text-muted-foreground"}`}
                      >
                        {plan.description}
                      </p>
                    )}
                    ...
                    {!(isTrial && isEnterprise) && (
                      <div className="mt-6">
                        {isCustomPlan ? (
                          <div
                            className={`text-3xl font-bold ${isEnterprise ? "text-white" : "text-foreground"}`}
                          >
                            {plan.billingTextKey
                          ...
                          <>
                            <span
                              className={`text-4xl font-bold ${isEnterprise ? "text-white" : "text-foreground"}`}
                            >
                              {plan.price}
                            </span>
                            <span
                    ...
                    {isCurrent && organization?.subscriptionStartDate && (
                      <div className="mt-4 space-y-1 text-sm">
                        <div
                          className={`${isEnterprise ? "text-white/80" : "text-foreground/80"}`}
                        >
                        ...
                        "pending_cancelled" ? (
                          ...
                          organization?.nextBillingDate && (
                            ...
                            >
                              ...
                              <span className="font-medium text-orange-600">
                                {format(
                                  new Date(organization.nextBillingDate),
                                  "dd.MM.yyyy",
                                )}
                              </span>
                            </div>
                  ...
                  <CardContent className="pt-0 flex flex-col flex-1">
                    <ul className="space-y-4 mb-8 flex-1">
                      {(plan?.featuresKeys && plan?.featuresKeys.length > 0
                        ...
                        <li
                          key={i}
                          className={`flex items-center ${isEnterprise ? "text-white" : "text-foreground"}`}
                        >
                          <Check className="w-5 h-5 text-green-500 mr-3 flex-shrink-0" />
                          <span className="text-sm">
                            {plan?.featuresKeys && plan?.featuresKeys.length > 0
        ...
        >
          <DialogContent className="sm:max-w-md">
            <DialogHeader>
              ...
              <DialogDescription>
                {t("organisation.enterpriseRequestDesc")}
              </DialogDescription>
            </DialogHeader>
        ...
        <Dialog
          ...
          onOpenChange={(open) =>
            !isDowngrading &&
            setDowngradeModal({ open, plan: open ? downgradeModal.plan : null })
          }
        >
          <DialogContent className="sm:max-w-md">
            <DialogHeader>
              ...
              <DialogDescription>
                {t("organisation.downgradeDesc")
                  .replace(
                    "{plan}",
                    downgradeModal.plan?.nameKey
                      ? t(`plan.${downgradeModal.plan.nameKey}`)
                      : downgradeModal.plan?.name || "",
                  )
                  .replace(
                    "{date}",
                    organization?.nextBillingDate
