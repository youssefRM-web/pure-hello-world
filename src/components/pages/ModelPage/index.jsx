...
const ModelPage = () => {
  ...
  useEffect(() => {
    ...
    if (!validSlugs.includes(slugId) && !slugId.startsWith('models')) {
      navigate('/');
    }

    const fetchProductVariants = async () => {
      if (slugId.startsWith('models')) {
        // Extract the modelName from the slugId
        const modelName = slugId.replace('models/', '');
        const productVariants = await getCollection({
          modelName, // Pass modelName as a parameter
        });
