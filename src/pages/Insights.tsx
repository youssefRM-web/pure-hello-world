...
const Insights = () => {
  ...
  const {
    ...
    queryKey: [
      ...
      organizationId,
      buildingFilter === "all" ? null : buildingFilter,
      dateRange,
    ],
    queryFn: async () => {
      if (!organizationId) {
      ...
      try {
        ...
        const response = await apiService.get<any>(
          endpoints.insightsComprehensive,
          { params },
        );
  ...
  const calculateStats = () => {
    ...
    const todoCount =
      taskStatus.find((s: any) => s.status === "TO_DO")?.count || 0;
    const inProgressCount =
      taskStatus.find((s: any) => s.status === "IN_PROGRESS")?.count || 0;
    const doneCount =
      taskStatus.find((s: any) => s.status === "DONE")?.count || 0;
  ...
  const calculatePriorityData = () => {
    ...
    const highCount =
      taskPriority.find((p: any) => p.priority === "High")?.count || 0;
    const mediumCount =
      taskPriority.find((p: any) => p.priority === "Medium")?.count || 0;
    const lowCount =
      taskPriority.find((p: any) => p.priority === "Low")?.count || 0;
    const noPriorityCount =
      taskPriority.find((p: any) => !p.priority || p.priority === "None")
        ?.count || 0;
  ...
  const totalOpenTasks = priorityChartData.reduce(
  ...
  const calculateCategoryData = () => {
    if (!insightsData?.categories || !Array.isArray(insightsData.categories))
      return [];
  ...
  const maxUserTasks = Math.max(
    ...(userData.length > 0 ? userData.map((user) => user.tasks) : [1]),
  );
  const maxUserAssigned = Math.max(
    ...(userData.length > 0 ? userData.map((user) => user.assigned) : [1]),
  );
  const maxCategoryCount = Math.max(
    ...(categoryData.length > 0 ? categoryData.map((cat) => cat.count) : [1]),
  );
  ...
  const handleExport = () => {
    try {
      const exportData = {
        ...
        priorities: priorityChartData.map((p) => ({
          priority: p.name,
          count: p.value,
        })),
  ...
  return (
    <div className="flex-1 flex flex-col h-full overflow-hidden">
      ...
      >
        ...
        {/* Scrollable Content */}
        <div className="flex-1 overflow-y-auto px-4 lg:px-6 pb-4 lg:pb-6 pt-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-5 gap-4 mb-8">
              {statsData.map((stat, index) => {
                ...
                return (
                  <Card
                    key={index}
                    className="relative overflow-hidden rounded-2xl border bg-card/80 backdrop-blur-sm shadow-sm hover:shadow-lg hover:-translate-y-0.5 transition-all duration-300 group"
                  >
                    <div
                    ...
                    <CardContent className="p-5 relative">
                      <div className="flex items-center justify-between mb-3">
                        ...
                        <div
                          className={`h-9 w-9 rounded-xl bg-background/80 backdrop-blur flex items-center justify-center shadow-sm ${accent.split(" ").pop()}`}
                        >
                ...
                <CardContent>
                  ...
                  <div className="flex justify-between mt-6 text-xs text-foreground">
                    ...
                    <span>
                      {Math.ceil(
                        Math.max(
                          ...highPriorityTickets.map((u) => u.value),
                          1,
                        ) / 3,
                      )}
                    </span>
                    ...
                    <span>
                      {Math.max(...highPriorityTickets.map((u) => u.value), 1)}
                    </span>
                  </div>
                </CardContent>
              </Card>
                        ...
                        <div key={index} className="grid grid-cols-2 gap-8">
                          <div className="flex items-center justify-between">
                            ...
                            <div className="flex items-center gap-2">
                              <div
                                ...
                                style={{
                                  width: `${Math.max((user.tasks / maxUserTasks) * 80, 8)}px`,
                                }}
                              ></div>
                          ...
                          <div className="flex items-center gap-2">
                            <div
                              ...
                              style={{
                                width: `${Math.max((user.assigned / maxUserAssigned) * 80, 8)}px`,
                              }}
                            ></div>
                            <span className="text-sm font-medium">
                              {user.assigned}
                        ...
                        >
                          ...
                          <div className="flex items-center gap-2">
                            <div
                              className="h-8 bg-blue-600 transition-all duration-300"
                              style={{
                                width: `${Math.max((cat.count / maxCategoryCount) * 80, 8)}px`,
                              }}
                            ></div>
                            <span className="text-sm font-medium">
                              {cat.count}
